...
# TODO: Ask Haleigh about the graphics since theyll be changing with the next installment
...
allparishes <- paste0("allparishesRaw", YEAR_PEP) %>%
...
Overallpop <- allparishes %>%
  filter(date == paste0("7/1/", YEAR_PEP, " population estimate")) %>% ## PEP update
  filter(sex == "Total") %>%
  filter(hisp == "Total") %>%
...
# TODO: Ask Haleigh what to do about this?
AA2023 <- AAhistorical %>%
  filter(year == 2023) %>%
  select(POP) %>%
  pull()
...
getvalue.bachdeg.nopct <- bach %>%
  filter(placename == "Orleans") %>%
  select(pctbach) %>%
  as.numeric()
getvalue.bachdeg.USpct <- bach %>%
  filter(placename == "United States") %>%
  select(pctbach) %>%
  as.numeric()
getvalue.bachdeg.metropct <- bach %>%
  filter(placename == "New Orleans Metro Area") %>%
  select(pctbach)

getvalue.bachdeg.white.nopct <- bach.race %>%
  filter(place.fac == "Orleans" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  as.numeric()
getvalue.bachdeg.black.nopct <- bach.race %>%
  filter(place.fac == "Orleans" & var.fac == "Black") %>%
  select(val) %>%
  as.numeric()

getvalue.bachdeg.white.maxotherpct <- bach.race %>%
  filter(place.fac != "Orleans" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  max() %>%
  as.numeric()

getvalue.bachdeg.black.jeffpct <- bach.race %>%
  filter(place.fac == "Jefferson" & var.fac == "Black") %>%
  select(val)
getvalue.bachdeg.black.stTampct <- bach.race %>%
  filter(place.fac == "St. Tammany" & var.fac == "Black") %>%
  select(val)

getvalue.noHSdeg.metropct <- hs %>%
  filter(placename == "New Orleans Metro Area") %>%
  select(pctless)
getvalue.noHSdeg.nopct <- hs %>%
  filter(placename == "Orleans") %>%
  select(pctless)
...
getvalue.medhh.black.no <- medhh.race %>%
  filter(place.fac == "Orleans" & var.fac == "Black") %>%
  select(val) %>%
  as.numeric()
getvalue.medhh.white.no <- medhh.race %>%
  filter(place.fac == "Orleans" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  as.numeric()
getvalue.medhh.hisp.no <- medhh.race %>%
  filter(place.fac == "Orleans" & var.fac == "Hispanic,\nany race") %>%
  select(val) %>%
  as.numeric()

getvalue.medhh.blkwht.no.pctdiff <- (getvalue.medhh.white.no - getvalue.medhh.black.no) / getvalue.medhh.white.no

getvalue.medhh.black.stTam <- medhh.race %>%
  filter(place.fac == "St. Tammany" & var.fac == "Black") %>%
  select(val) %>%
  as.numeric()
getvalue.medhh.white.stTam <- medhh.race %>%
  filter(place.fac == "St. Tammany" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  as.numeric()
...
getvalue.medhh.hisp.metro <- medhh.race %>%
  filter(place.fac == "Metro" & var.fac == "Hispanic,\nany race") %>%
  select(val) %>%
  as.numeric()

getvalue.medhh.black.jeff <- medhh.race %>%
  filter(place.fac == "Jefferson" & var.fac == "Black") %>%
  select(val) %>%
  as.numeric()
getvalue.medhh.white.jeff <- medhh.race %>%
  filter(place.fac == "Jefferson" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  as.numeric()
...
getvalue.pov.black.metropct <- pov.race %>%
  filter(place.fac == "Metro" & var.fac == "Black") %>%
  select(val) %>%
  as.numeric()
getvalue.pov.hisp.metropct <- pov.race %>%
  filter(place.fac == "Metro" & var.fac == "Hispanic,\nany race") %>%
  select(val) %>%
  as.numeric()
getvalue.pov.white.metropct <- pov.race %>%
  filter(place.fac == "Metro" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  as.numeric()
getvalue.pov.asian.metropct <- pov.race %>%
  filter(place.fac == "Metro" & var.fac == "Asian") %>%
  select(val) %>%
  as.numeric()
...
getvalue.childpov.white.nopct <- childpov.race %>%
  filter(place.fac == "Orleans" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  as.numeric()
getvalue.childpov.hisp.nopct <- childpov.race %>%
  filter(place.fac == "Orleans" & var.fac == "Hispanic,\nany race") %>%
  select(val) %>%
  as.numeric()
getvalue.childpov.hisp.metropct <- childpov.race %>%
  filter(place.fac == "Metro" & var.fac == "Hispanic,\nany race") %>%
  select(val) %>%
  as.numeric()
...
getvalue.homeowner.white.metropct <- ho.race %>%
  filter(place.fac == "Metro" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  as.numeric()
getvalue.homeowner.black.metropct <- ho.race %>%
  filter(place.fac == "Metro" & var.fac == "Black") %>%
  select(val) %>%
  as.numeric()

getvalue.homeowner.black.stTam <- ho.race %>%
  filter(place.fac == "St. Tammany" & var.fac == "Black") %>%
  select(val) %>%
  as.numeric()
getvalue.homeowner.white.stTam <- ho.race %>%
  filter(place.fac == "St. Tammany" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  as.numeric()
getvalue.homeowner.hisp.stTam <- ho.race %>%
  filter(place.fac == "St. Tammany" & var.fac == "Hispanic,\nany race") %>%
  select(val) %>%
  as.numeric()

getvalue.homeowner.white.nopct <- ho.race %>%
  filter(place.fac == "Orleans" & var.fac == "White,\nnon-Hispanic") %>%
  select(val) %>%
  as.numeric()
getvalue.homeowner.black.nopct <- ho.race %>%
  filter(place.fac == "Orleans" & var.fac == "Black") %>%
  select(val) %>%
  as.numeric()
